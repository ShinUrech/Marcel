...
import NewsCardItemSm from './NewsCardItemSm';
...
const ReadNext = () => {
    ...
    const fetchArticles = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/articles/nrandom`,
          {
  ...
  return (
    <div className="flex flex-col border-t pt-15">
      <h3 className="font-bold capitalize">{t('readNext')}</h3>
      <div className="grid md:grid-cols-2 gap-5 lg:grid-cols-4 sm:grid-cols-1">
        {articles.map((article) => (
          <NewsCardItemSm key={article._id} article={article} />
        ))}
      </div>
    </div>
  );
