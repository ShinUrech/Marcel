...
function getLinkedInCredentials() {
  ...
  // Backward-compat safety: treat placeholder values as "not configured"
  if (!email || email === 'TO REPLACE' || !password || password === 'TO REPLACE') {
    throw new Error(
      'LinkedIn credentials are not set. Please set LINKEDIN_EMAIL and LINKEDIN_PASSWORD in .env.development (or your runtime environment) and restart the server.',
    );
  }
  ...
  try {
    ...
    if (isLoggedIn) {
      ...
      if (hasUsername && hasPassword) {
        ...
        await page
          ...
          .catch((e) => console.log(`[LinkedIn] Login navigation wait ended: ${e.message}`));
    ...
    const articles = await page.evaluate((articleType) => {
      ...
      for (const post of posts) {
        ...
        const dateText = dateEl ? (dateEl as HTMLElement).innerText.trim() : '';

        // Simple relative date parsing logic inside browser context
        // Note: This is a simplified version. Ideally, we should use the helper function,
        // but we can't import it easily into page.evaluate.
        // We will filter strictly in the Node.js context, but we can try to stop early here if needed.
        // For now, let's extract everything and filter in Node.js to be safe and use the robust parser.
        ...
        const imageElement =
          post.querySelector('.update-components-image__image') || post.querySelector('.feed-shared-image__image');
        ...
        if (textElement || imageElement) {
          ...
          const contentText = textElement ? (textElement as HTMLElement).innerText : '';
          ...
          results.push({
            ...
            url: postUrl,
            dateText: dateText,
            image: imageElement ? (imageElement as HTMLImageElement).src : 'N/A',
    ...
    // Filter articles based on date in Node.js context where we have the helper
    // const { parseRelativeDateLinkedIn } = require('../../../../common/utils/format-date');
    ...
    for (const article of articles) {
      ...
      if (fromDate) {
        // Set article date to end of day for comparison if it's just a date without time
        // But parseRelativeDateLinkedIn usually returns a Date object with time if "hours ago"
        // If it's "1d", it returns yesterday same time.
        // Let's be safe and compare timestamps.
